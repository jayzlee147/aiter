...
def make_dense_inputs(
    ...
    *,
    ...
    v = (
        torch.randn(B, T, H, D, dtype=torch.bfloat16, device="cuda") * 0.5
    )
    g = F.logsigmoid(
        torch.randn(B, T, H, dtype=torch.float32, device="cuda")
    )
    beta = torch.randn(
        B, T, H, dtype=torch.float32, device="cuda"
    ).sigmoid()
...
@pytest.mark.parametrize(
    ("B", "T", "H"),
    [pytest.param(*case, id=f"B{case[0]}-T{case[1]}-H{case[2]}") for case in DENSE_CASES],
)
@pytest.mark.parametrize(
...
def test_opus_gdn_c_prefill_vs_reference(
    ...
    require_c_runtime()
    q, k, v, g, beta, state_vk, ref_output, ref_final_kv = (
        case_with_reference(
...
def test_opus_gdn_c_prefill_reuses_preallocated_output(c_mode: int):
    ...
    output, final = opus_gdn_c_prefill_fwd(
        q, k, v, g, beta, c_mode=c_mode, out=out
    )
