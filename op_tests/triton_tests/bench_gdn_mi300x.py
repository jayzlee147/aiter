#!/usr/bin/env python3
"""GDN Prefill benchmark: Triton (aiterlatest) vs Opus (HIP fused) on MI300X (gfx942).
...
def generate_report(dense_results, varlen_results, bt_results, device_name):
    lines = []
    lines.append("# Gated-DeltaNet Prefill: Triton vs Opus Fused — MI300X (gfx942) 性能对比报告\n")
    lines.append(f"## 测试环境\n")
    lines.append(f"- **设备**: AMD Instinct **{device_name}** (gfx942 / CDNA3), 304 CU, wave64, 512 VGPR/lane/SIMD, 64 KB LDS/CU")
    ...
    lines.append(f"4. **gfx942 特有**: 无 32x32x16 MFMA; split-K2 (ref_fwd_h) OCC=1, 已验证更慢。\n")
    ...
    # MI300X specifics
    lines.append("## 5. MI300X (gfx942) 资源与占用\n")
    ...
    lines.append("")
    lines.append("### gfx942 特殊限制\n")
    lines.append("- **无 32x32x16 MFMA**: split-K2 的 ref_fwd_h 只能用 16x16x16, 效率降低")
    lines.append("- **ref_fwd_h OCC=1**: VGPR+AGPR=264 → 仅 1 wave/SIMD, 已验证 split-K2 更慢 (1453us vs fused 1388us)")
    lines.append("- **Wavefront fused 更慢**: AGPR 128→132, OCC 4→3, 原子同步开销, 验证无收益")
    lines.append("- **64KB LDS**: BT=128 不可用 (超限)")
    lines.append("")
    ...
    lines.append("## 复现\n")
    lines.append("```bash")
    lines.append("cd /home/sijieli2")
    lines.append("python bench_gdn_mi300x.py")
    lines.append("```\n")
...
def main():
    ...
    report_path = "/home/sijieli2/bench_gdn_mi300x_report.md"
    ...
    json_path = "/home/sijieli2/bench_gdn_mi300x_raw.json"
    def default_ser(o):
