// Experimental eight-wave P3 with cross-segment Kd prefetch for gfx942.
//
...
__device__ __forceinline__ constexpr int bt64_bv16_nw8_pf_tri_tile(
        int r, int c) {
    return r * (r + 1) / 2 + c;
}
...
__device__ __forceinline__ f32x4 bt64_bv16_nw8_pf_contract_last(
...
template <bool HI, bool HO, bool STATE_IN_FP32, bool STATE_OUT_FP32,
          bool VL, bool SWIZZLE, bool SEGMENT_RANGE>
__device__ __forceinline__ void
k2_kda_csplit_bt64_bv16_nw8_prefetch_body(
    ...
    const int bh = SWIZZLE ? int(blockIdx.x) / (D / BV) : int(blockIdx.x);
    const int v0 = SWIZZLE
        ? (int(blockIdx.x) % (D / BV)) * BV : int(blockIdx.y) * BV;
    ...
    for (int s = scan_begin; s < scan_end; ++s) {
        ...
        *reinterpret_cast<bf16x4*>(
        ...
        if (wave < 4) {
            ...
            for (int j = 0; j < 4; ++j) {
                if (j <= wave) {
                    const f32x4 x = bt64_bv16_nw8_pf_contract_last<C, C, ST>(
                        cinv + bt64_bv16_nw8_pf_tri_tile(wave, j) * C * C,
                        vT + j * C, lane);
                    #pragma unroll
                    for (int i = 0; i < 4; ++i)
                        u[i] += x[i];
        ...
        for (int rb = 0; rb < 4; ++rb) {
            const f32x4 c = bt64_bv16_nw8_pf_contract_last<C, ST, ST>(
                krT + wave * C * ST + rb * C,
                uT + rb * C, lane);
...
k2_kda_csplit_bt64_bv16_nw8_prefetch_kernel(
        const __bf16* __restrict__ v_g,
        const float* __restrict__ beta_src,
        const __bf16* __restrict__ ws_kd,
        const __bf16* __restrict__ ws_kr,
    ...
    (void)segment_ready;
    k2_kda_csplit_bt64_bv16_nw8_prefetch_body<
        HI, HO, SFP32, SFP32, VL, SWIZZLE, false>(
...
k2_kda_csplit_bt64_bv16_nw8_prefetch_range_kernel(
        ...
        int segment_begin, int segment_count) {
    k2_kda_csplit_bt64_bv16_nw8_prefetch_body<
        HI, HO, STATE_IN_FP32, STATE_OUT_FP32, VL, false, true>(
            v_g, beta_src, ws_kd, ws_kr, ws_gt, ws_decay, ws_inv,
            cross32, cross64, cs_u, cs_sin, init_state, final_state,
            cu_seqlens, tile_prefix, pair_prefix, segment_prefix,
            total_tiles, total_pairs, total_segments, T_seq, H, NT,
            scan_flags, segment_begin, segment_count);
}
