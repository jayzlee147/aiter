// Experimental compact-worklist eight-wave P3 consumer for gfx942.
//
...
__device__ __forceinline__ constexpr int bt64_bv16_nw8_wl_tri_tile(
        int r, int c) {
    return r * (r + 1) / 2 + c;
}
...
__device__ __forceinline__ f32x4 bt64_bv16_nw8_wl_contract_last(
...
template <bool HI, bool HO, bool STATE_IN_FP32, bool STATE_OUT_FP32,
          bool VL, bool SWIZZLE, bool SEGMENT_RANGE, bool BOUNDED,
          bool TASK_OVERRIDE>
__device__ __forceinline__ void
k2_kda_csplit_bt64_bv16_nw8_worklist_body(
        ...
        const int* __restrict__ sequence_counts,
        int sequence_count_index,
        ...
        int work_bh_override, int work_vtile_override) {
    constexpr int C = 16;
    constexpr int BT = 64;
    constexpr int D = 128;
    constexpr int BV = 16;
    ...
    // The optional 1-D mapping launches all eight V16 consumers of one head
    // consecutively, improving the chance that their repeated Kd/Kr/C reads
    // meet in L2.  It is standalone-only until measured on the target GPU.
    const int work_bh = TASK_OVERRIDE
        ? work_bh_override
        : (SWIZZLE ? int(blockIdx.x) / (D / BV) : int(blockIdx.x));
    const int work_seq = work_bh / H;
    if constexpr (BOUNDED) {
        if (work_seq >= sequence_counts[sequence_count_index])
            return;
    }
    const int mapped_seq = sequence_worklist[work_seq];
    const int bh = mapped_seq * H + work_bh % H;
    const int v0 = (TASK_OVERRIDE
        ? work_vtile_override
        : (SWIZZLE ? int(blockIdx.x) % (D / BV) : int(blockIdx.y))) * BV;
    ...
    for (int s = scan_begin; s < scan_end; ++s) {
        ...
        *reinterpret_cast<bf16x4*>(
        ...
        // Segment zero pays the compulsory Kd load.  Every later segment was
        // fetched by the previous iteration while its MFMA chain was active.
        // The full BT64xK128 tile is exactly two bf16x8 packs per thread, so
        // this costs eight VGPRs and preserves the existing LDS/MFMA layout.
        if (s == scan_begin) {
        ...
        // Issue s+1 before any current-segment MFMA.  The packs are not
        // consumed until the end of the iteration, leaving Kd@H, C@R, and
        // Kr^T@U to cover their VMEM latency.
        bf16x8 kd_next[2]{};
        if (s + 1 < scan_end) {
            const int next_alen = min(BT, seq_len - (s + 1) * BT);
            #pragma unroll
            for (int p = 0; p < 2; ++p) {
                const int vi = tid + p * 512;
                const int m = vi / (D / 8);
                const int k8 = (vi % (D / 8)) * 8;
                if (m < next_alen) {
                    const int rb = m >> 4;
                    const int r = m & 15;
                    kd_next[p] = *reinterpret_cast<const bf16x8*>(
                        ws_kd +
                        (int64_t(ht0 + 4 + rb) * C + r) * D + k8);
                }
            }
        }
        ...
        if (wave < 4) {
            ...
            for (int j = 0; j < 4; ++j) {
                if (j <= wave) {
                    const f32x4 x = bt64_bv16_nw8_wl_contract_last<C, C, ST>(
                        cinv + bt64_bv16_nw8_wl_tri_tile(wave, j) * C * C,
                        vT + j * C, lane);
                    #pragma unroll
                    for (int i = 0; i < 4; ++i)
                        u[i] += x[i];
        ...
        for (int rb = 0; rb < 4; ++rb) {
            const f32x4 c = bt64_bv16_nw8_wl_contract_last<C, ST, ST>(
                krT + wave * C * ST + rb * C,
                uT + rb * C, lane);
        ...
        // Seed the next iteration's Kd pool after every old-phase consumer has
        // retired.  Its normal top-of-loop barrier also publishes these stores,
        // so prefetching does not add a CTA synchronization point.
        if (s + 1 < scan_end) {
            #pragma unroll
            for (int p = 0; p < 2; ++p) {
                const int vi = tid + p * 512;
                const int m = vi / (D / 8);
                const int k8 = (vi % (D / 8)) * 8;
                *reinterpret_cast<bf16x8*>(kd + m * SK + k8) = kd_next[p];
            }
        }
    }
...
template <bool HI = false, bool HO = false,
          bool SFP32 = false, bool VL = false, bool SWIZZLE = false,
          bool BOUNDED = false>
__global__ void __launch_bounds__(512)
k2_kda_csplit_bt64_bv16_nw8_worklist_kernel(
        const __bf16* __restrict__ v_g,
        const float* __restrict__ beta_src,
        const __bf16* __restrict__ ws_kd,
        const __bf16* __restrict__ ws_kr,
        ...
        int total_tiles, int total_pairs, int total_segments,
        int T_seq, int H, int NT, unsigned scan_flags,
        unsigned int* __restrict__ segment_ready = nullptr) {
    (void)segment_ready;
    k2_kda_csplit_bt64_bv16_nw8_worklist_body<
        HI, HO, SFP32, SFP32, VL, SWIZZLE, false, BOUNDED, false>(
            ...
            sequence_worklist, sequence_counts, sequence_count_index,
...
k2_kda_csplit_bt64_bv16_nw8_persistent_worklist_kernel(
    ...
    __shared__ unsigned int task_shared;
    while (true) {
        if (threadIdx.x == 0)
            task_shared = atomicAdd(task_counter, 1u);
        __syncthreads();
        const int task = int(task_shared);
        const int task_count =
            sequence_counts[sequence_count_index] * H * V_TILES;
        if (task >= task_count)
            return;
        const int work_bh = task / V_TILES;
        const int work_vtile = task % V_TILES;
        k2_kda_csplit_bt64_bv16_nw8_worklist_body<
            HI, HO, SFP32, SFP32, true, false, false, false, true>(
                v_g, beta_src, ws_kd, ws_kr, ws_gt, ws_decay, ws_inv,
                cross32, cross64, cs_u, cs_sin, init_state, final_state,
                cu_seqlens, tile_prefix, pair_prefix, segment_prefix,
                sequence_worklist, sequence_counts, sequence_count_index,
                total_tiles, total_pairs, total_segments, T_seq, H, NT,
                scan_flags, 0, 0, work_bh, work_vtile);
        __syncthreads();
    }
}
