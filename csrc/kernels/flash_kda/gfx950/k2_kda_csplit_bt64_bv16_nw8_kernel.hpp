// gfx950 eight-wave low-LDS RTP BT64 recurrent scan.
//
// This is an arch-private operator rather than a runtime branch inside the
// gfx942 kernel.  Its K128 Kd@H^T contraction uses gfx950's native x32 BF16
// MFMA while the K16 C@R and Kr^T@V' chains retain their established x16 form.
//
...
template <bool HI = false, bool HO = false,
          bool SFP32 = false, bool VL = false, int BV = 16>
__global__ void __launch_bounds__(512)
k2_kda_csplit_bt64_bv16_nw8_x32_kernel(
    ...
    // BV8 retains a full N16 MFMA tile.  The upper eight columns are explicit
    // zeros, trading half of each MFMA's N work for twice as many independent
    // CTAs when a single sequence cannot fill gfx950's 256 CUs.
    constexpr int PHASE_ELEMS = (MFMA_N + BT) * SK;
    constexpr int VT_ELEMS = MFMA_N * ST;
    ...
    static_assert(BV == 8 || BV == 16,
                  "gfx950 NW8 scan supports logical BV8 or BV16");
    ...
    __bf16* const kd = phase + MFMA_N * SK;
    ...
    for (int s = 0; s < ns; ++s) {
        ...
        *reinterpret_cast<bf16x4*>(hT + lv * SK + kl) = hpack;
        ...
        if (wave < 4) {
            ...
            if (valid_v && m0 + 1 < alen)
                v_prefetch1 = v_g[
                    (int64_t(t0 + m0 + 1) * H + h) * D + v0 + vv];
            if (valid_v && m0 + 2 < alen)
                v_prefetch2 = v_g[
                    (int64_t(t0 + m0 + 2) * H + h) * D + v0 + vv];
            if (valid_v && m0 + 3 < alen)
                v_prefetch3 = v_g[
        ...
        if (wave < 4) {
            const f32x4 p0 = contract_last_x32<64, SK, SK>(
                kd + wave * C * SK, hT, lane);
            const f32x4 p1 = contract_last_x32<64, SK, SK>(
        ...
        if (wave < 4) {
            ...
            for (int i = 0; i < 4; ++i) {
                ...
                const float value = valid_v && m < alen
                    ? bf16_to_f32(v_prefetch)
                    : 0.0f;
                vT[vv * ST + m] = valid_v && m < alen
