...
gdn_k2_out_kernel(gdn_k2_kargs kargs) {
    ...
    // Map the logical chunk before deriving any token/snapshot address.  K6
    // chunks are independent, so the reverse variant changes scheduling only.
    const int i_t  = T::REVERSE_CHUNKS
        ? (kargs.NT - 1 - static_cast<int>(blockIdx.y))
