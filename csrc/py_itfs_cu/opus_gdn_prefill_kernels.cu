...
template<typename Traits>
__global__ void gdn_k2_bt32_kernel(gdn_k2_kargs kargs);
    ...
    bool output_final_state)
{
    ...
    k2args.ptr_h_snap  = nullptr;  // forward-only: skip h snapshot write
    k2args.ptr_v_new   = nullptr;  // forward-only: skip v_new HBM write
...
template<typename K1Traits, typename K2Traits>
void launch_gdn_prefill_neumann(
    torch::Tensor q,
    torch::Tensor k,
    torch::Tensor v,
    torch::Tensor g,
    ...
    bool output_final_state)
{
    ...
    gdn_k2_bt32_kernel<K2Traits><<<k2_grid, k2_block, k2_smem, stream>>>(k2args);
}
...
#define DISPATCH_K2(bt, bv, nw) \
    launch_gdn_prefill<gdn_k1_traits<bt, 128, 128, 4>, \
                       gdn_k2_traits<bt, 128, 128, bv, nw>>( \
        q, k, v, g, beta, o, scale, \
        initial_state, final_state, has_initial_state, output_final_state)

#define DISPATCH_K2_NEUMANN(bt, bv, nw) \
    launch_gdn_prefill_neumann<gdn_k1_traits<bt, 128, 128, 4>, \
                               gdn_k2_traits<bt, 128, 128, bv, nw>>( \
        q, k, v, g, beta, o, scale, \
        initial_state, final_state, has_initial_state, output_final_state)
    ...
    if (BT == 64 && k1_algo == 1) {
        if (BV == 64 && num_warps == 4) {
            DISPATCH_K2_NEUMANN(64, 64, 4);
        } else if (BV == 32 && num_warps == 4) {
            DISPATCH_K2_NEUMANN(64, 32, 4);
        } else if (BV == 64 && num_warps == 8) {
            DISPATCH_K2_NEUMANN(64, 64, 8);
        } else if (BV == 32 && num_warps == 8) {
            DISPATCH_K2_NEUMANN(64, 32, 8);
        } else {
        ...
        if (BV == 64 && num_warps == 4) {
            launch_gdn_prefill_bt32<gdn_k1_traits<32, 128, 128, 4>,
                                    gdn_k2_traits<32, 128, 128, 64, 4>>(
                q, k, v, g, beta, o, scale,
                initial_state, final_state, has_initial_state, output_final_state);
        } else {
