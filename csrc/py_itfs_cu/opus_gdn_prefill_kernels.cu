...
// Wavefront-pipelined h-scan: super-chunk grid dim + inter-WG atomic handoff
// (adds the chunk axis to the grid to fill idle CUs; gfx942 grid-starvation fix).
template<typename Traits>
    ...
    float scale)
{
    ...
    constexpr int smem_vT = BV * STRIDE_BT * (int)sizeof(__bf16);
    constexpr int pool_bc = BT * STRIDE_BK * (int)sizeof(__bf16);
    constexpr int pool_d  = K2Traits::BK_SUB * STRIDE_BT * (int)sizeof(__bf16);
    constexpr int pool_h  = K2Traits::BK_SUB * BV * (int)sizeof(__bf16); // h LDS staging (bf16)
