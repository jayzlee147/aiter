    ...
    * c_mode=0: a conservative policy measured on an 80-CU gfx942. CS is
    ...
    Args:
        ...
        out: Optional preallocated contiguous bf16 output. Preallocation
            requires T to be divisible by 64, and out must not alias v.
    ...
    device_gfx = torch.cuda.get_device_properties(q.device).gcnArchName.split(
        ":", 1
    )[0]
    ...
    if out is not None:
        if pad_len:
            raise ValueError(
                "a preallocated output requires T to be divisible by 64"
            )
        ...
        if (
            ...
            raise ValueError("out must not alias v storage")
    ...
    output = (
        ...
        if out is not None
            ...
            (B, padded_T, H, 128), dtype=torch.bfloat16, device=q.device
        )
    )
